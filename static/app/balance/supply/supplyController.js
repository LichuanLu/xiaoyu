		 ...
		 supplyService, userIdService,thirdPayService,validateMessage) {
			...
			$scope.aliPayAction = function(activityId) {
				...
				thirdPayService.getAliPayLink(userIdService.getData(),
					$scope.orderId, 0, 1, $scope.countPrice,activityId).then(function(data) {
			...
			$scope.wechatAction = function(activityId) {
				var url = thirdPayService.getWeChatPayLink(userIdService.getData(), $scope.orderId, 0, 2, $scope.countPrice,activityId);
				if (url) {
